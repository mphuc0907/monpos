...
//        $id = $_POST['id'];
        ...
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
//        if ($id == 1) {
//            $users = $this->db->select('users.id, username, email, display_name, user_status, group_name ')->from('users')->join('users_group', 'users_group.id = users.group_id')->where('users.parent_id', $parent)->get()->result_array();
//
//        } else {
//
//        }
        $users = $this->db->select('users.id, username, email, display_name, user_status, group_name ')->from('users')->join('users_group', 'users_group.id = users.group_id')->where('users.parent_id', $parent)->or_where('users.id', $parent)->get()->result_array();
        if (!empty($users) && count($users) != 0) {
            $ind = 0;
            $html = '';
            foreach ($users as $user) {
                $text = '';
                if($store_id['group_id'] == 1){
                    if($user['group_name'] == 'Ban giám đốc'){
                        $text = 'Khách hàng';
                    }else{
                ...
                $html .= '<td>' . '<span class="user_group"><i class="fa fa-male"></i> ' .$text . '</span>' . '</td>';
    ...
    public function cms_save_item_user()
    {
        ...
        if (!isset($user) && count($user) == 0) {
            ...
            if ($data['email'] == $user['email']) {
                ...
                $user_list = $this->db->where('parent_id', $id)->from('users')->get()->result_array();

                foreach ($user_list as $val) {
                    $val['user_status'] = ($data['user_status'] == 0) ? $data['user_status'] : $val['user_status'];
                ...
                if ($this->_check_mail($data['email'])) {
                    ...
                    $data['updated'] = gmdate("Y:m:d H:i:s", time() + 7 * 3600);
                    $this->db->where('id', $id)->update('users', $data);
                    $user_list = $this->db->where('parent_id', $data['id'])->from('users')->get()->result_array();
                    foreach ($user_list as $val) {
                        $val['user_status'] = ($data['user_status'] == 0) ? $data['user_status'] : $val['user_status'];
                        $this->db->where('id', $val['id'])->update('users', $val);
                    }
    ...
    //setting click icon delete, xoa nhan vien
    public function cms_del_usitem()
    {
        ...
        if (!isset($user) || count($user) == 0) {
            ...
            //
            $this->db->where(['id' => $id])->or_where('parent_id', $id)->delete('users');
            echo $this->messages = '1';
        }
        die();
    }
    ...
    public function cms_upgroup()
    {
        ...
        if (!empty($groups) && count($groups) != 0) {
            ...
            foreach ($groups as $group) {
                ...
                $html .= '<td class="text-center"><i class="fa fa-pencil-square-o edit-item hidden" title="Sửa" onclick="cms_edit_gritem(' . $group['id'] . ')" style="margin-right: 10px; cursor: pointer;"></i><i onclick="cms_del_gritem(' . $group['id'] . ' )" title="Xóa" class="fa fa-trash-o delete-item" style="cursor: pointer;"></i></td>';
                $html .= '</tr>';
    ...
    public function cms_upstore()
    {
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $stores = $this->db->from('stores')->where('shop_id', $parent)->get()->result_array();
        if (!empty($stores) && count($stores) != 0) {
            ...
            foreach ($stores as $store) {
                ...
                $html .= '<td class="text-center ind">' . $ind . '</td>';
                $html .= '<td><input type="text" value="' . $store['stock_name'] . '" class="form-control" id="stock_name_input"></td>';
                $html .= '<td>' . $store['created'] . '</td>';
                $html .= '<td><i onclick="cms_uptore(' . $store['ID'] . ' )" title="Sửa" class="fa fa-gears " style="cursor: pointer;"></i>| |<i onclick="cms_deltore(' . $store['ID'] . ' )" title="Xóa" class="fa fa-trash-o delete-item " style="cursor: pointer;"></i></td>';
                $html .= '</tr>';
            }
            echo $this->messages = $html;
    ...
    public function cms_deltore()
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $group = $this->db->where('ID', $gid)->from('stores')->get()->row_array();
        $prod = $this->db->where('store_id', $gid)->where('shop_id', $parent)->from('inventory')->get()->row_array();
        if (!isset($group) || count($group) == 0) {
            $this->cms_common_string->cms_jsredirect('Thao tác không thành công!', CMS_BASE_URL . 'config');
        } elseif (count($prod) > 0){
            echo $this->messages = '0';
            return;
        }else {
            $this->db->where(['ID' => $gid])->delete('stores');
            echo $this->messages = '1';
        }
        die();
    }

    public function cms_up_store()
    {
        $gid = $this->input->post('id');
        $name_st = $this->input->post('stock_name_input');
        $gid = (int)$gid;
        $group = $this->db->where('ID', $gid)->from('stores')->get()->row_array();
        if (!isset($group) || count($group) == 0) {
            $this->cms_common_string->cms_jsredirect('Thao tác không thành công!', CMS_BASE_URL . 'config');
        } else {
            $this->db->where(['ID' => $gid])->update('stores',['stock_name' => $name_st]);
            echo $this->messages = '1';
        }
        die();
    }
    ...
    public function cms_selboxgroup()
    {
        ...
        if (isset($groups) && count($groups)) {
            ...
            foreach ($groups as $group) {
                if ($id != 1) {
                    if ($group['id'] != 1) {
                        $html .= '<option class="option_id_' . $group['id'] . '" value="' . $group['id'] . '">' . $group['group_name'] . '</option>';
                    }
                } else {
                    if ($group['id'] == 1 ) {
                        $html .= '<option class="option_id_' . $group['id'] . '" value="' . $group['id'] . '">' . $group['group_name'] . '</option>';
                    }elseif($group['id'] == 4){
                        $html .= '<option class="option_id_' . $group['id'] . '" value="' . $group['id'] . '"> Khách hàng</option>';
                    }
                }
