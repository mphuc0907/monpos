...
class Orders extends CI_Controller
{
    ...
    public function cms_print_order()
    {
        ...
        $datausers = $this->db->from('users')->where('id', $data_order['user_init'])->get()->row_array();
        ...
        $data_template['content'] = str_replace("{Ten_Nhan_Vien}", $datausers['display_name'], $data_template['content']);
    ...
    public function cms_update_order()
    {
        if ($this->auth == null) $this->cms_common_string->cms_redirect(CMS_BASE_URL . 'backend');
        $id = $this->input->post('id');
        $order = $this->db->from('orders')->where('ID', $id)->get()->row_array();
    ...
    public function cms_paylack()
    {
        ...
        if(!empty($data['status'])) {
            if($data['status'] == 1) {
                $order = $this->db->from('orders')->where('ID', $data['id'])->where('shop_id', $parent)->get()->row_array();

                if (isset($order) && count($order)) {
                    $lack = $order['lack'] - $data['lack_pay'];
                    $custom_pay = $order['customer_pay'] + $data['lack_pay'];
                    $this->db->where('ID', $data['id'])->where('shop_id', $parent)->update('orders', ['lack' => $lack, 'customer_pay' => $custom_pay]);
                    $order = $this->db->from('orders')->where('ID', $data['id'])->where('shop_id', $parent)->get()->row_array();
                ...
                $input =  $this->db->from('input')->where('ID', $data['id'])->where('shop_id', $parent)->get()->row_array();

                if (isset($input) && count($input)) {
                    $lack = $input['lack'] - $data['lack_pay'];
                    $custom_pay = $input['payed'] + $data['lack_pay'];
                    $this->db->where('ID', $data['id'])->where('shop_id', $parent)->update('input', ['lack' => $lack, 'payed' => $custom_pay]);
                    $input =  $this->db->from('input')->where('ID', $data['id'])->where('shop_id', $parent)->get()->row_array();
    ...
    public function cms_search_box_customer()
    {
        ...
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $data = $this->input->post('data');
        $customer_s = $this->db->like('customer_name', $data['keyword'])->or_like('customer_phone', $data['keyword'])->or_like('customer_email', $data['keyword'])->or_like('customer_code', $data['keyword'])->from('customers')->get()->result_array();
        $customer = array();
        if(!empty($customer_s)){
            foreach ($customer_s as $item){
    ...
    public function cms_select_product()
    {
        ...
        if (isset($product) && count($product) != 0) {
            ...
            <tr data-id="<?php echo $product['ID']; ?>">
                <td class="text-center seq order123"><?php echo $seq; ?></td>
                <td><?php echo $product['prd_code']; ?></td>
                <td><?php echo $product['prd_name']; ?></td>

                <td class="text-center hidden app-controlles-orders.php">
                    <?php echo (!empty($product['prd_image_url'])) ? "<img src='{$product['prd_image_url']}' alt='{$product['prd_name']}' width='50' height='50'/>" : '<i class="fa fa-cloud-upload-" style="font-size: 18px; color: #337ab7; cursor: pointer; "></i>' ?>
                </td>
