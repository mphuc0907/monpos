...
class Import extends CI_Controller
{
    ...
    public function cms_select_product()
    {
        ...
        if (isset($product) && count($product) != 0) {
            ...
            <tr data-id="<?php echo $product['ID']; ?>">
                <td class="text-center seq import"><?php echo $seq; ?></td>
    ...
    {
        $shop_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($shop_id['group_id'] == 1) ? 1 : (($shop_id['parent_id'] == 1) ? $shop_id['id'] : $shop_id['parent_id']);
//        if($this->auth['store_id'] == $store_id){
        $input = $this->input->post('data');
        $detail_input_temp = $input['detail_input'];
        $store_id = $input['store_id'];
//        print_r($detail_input_temp);die();
