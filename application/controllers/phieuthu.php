...
class Phieuthu extends CI_Controller
{
    private $auth;
    ...
    public function index($page = 1)
    {
        ...
        $total_thuchi = $this->db->from('thuchi')->where('shop_id', $parent)->where("loaiphieu", "phieuthu")->count_all_results();
        $config['base_url'] = 'cms_paging_listthuchithu';
        ...
        $data['template'] = 'phieuthu/index';
        $this->load->view('layout/index', isset($data) ? $data : null);
    }

    //phieu chi
    public function cms_phieuchithu()
    {
        ...
        if ($data['thuchi_code'] == '') {
            $this->db->select_max('thuchi_code')->where('shop_id', $parent)->like('thuchi_code', 'PT');
            $max_thuchi_code = $this->db->get('thuchi')->row();
            $max_code = (int)(str_replace('PT', '', $max_thuchi_code->thuchi_code)) + 1;
            if ($max_code < 10)
                $data['thuchi_code'] = 'PT00000' . ($max_code);
            else if ($max_code < 100)
                $data['thuchi_code'] = 'PT0000' . ($max_code);
            else if ($max_code < 1000)
                $data['thuchi_code'] = 'PT000' . ($max_code);
            else if ($max_code < 10000)
                $data['thuchi_code'] = 'PT00' . ($max_code);
            else if ($max_code < 100000)
                $data['thuchi_code'] = 'PT0' . ($max_code);
            else if ($max_code < 1000000)
                $data['thuchi_code'] = 'PT' . ($max_code);
    ...
    public function cms_paging_listthuchithu($page = 1)
    {
        $config = $this->cms_common->cms_pagination_custom();
        ...
        $this->load->view('phieuthu/list_phieuchiajax2', isset($data) ? $data : null);
    }


    public function cms_delThuchiThu()
    {
        $usser_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($usser_id['group_id'] == 1) ? 1 : (($usser_id['parent_id'] == 1) ? $usser_id['id'] : $usser_id['parent_id']);
        $id = (int)$this->input->post('id');
        $thuchi = $this->db->from('thuchi')->where('shop_id', $parent)->where('ID', $id)->get()->row_array();
