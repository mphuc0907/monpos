...
class Dashboard extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $today = date('Y-m-d');
        $orders = $this->db->from('orders')->where('shop_id',$parent)->where(('sell_date >'), $today)->where(('deleted'), 0)->get()->result_array();
        $tongtien = 0;
        $soluongsp = array();
        ...
        $data['slsinventory'] = count($this->db->select('ID')->where(['prd_status' => 1, 'deleted' => 0, 'prd_sls >' => 0])->where('shop_id',$parent)->from('products')->get()->result_array());
        $data['slsaceitem'] = count($this->db->select('ID')->where(['prd_status' => 1, 'deleted' => 0, 'prd_sls' => 0])->where('shop_id',$parent)->from('products')->get()->result_array());
