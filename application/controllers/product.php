...
//        print_r($this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array());die();
        ...
        $manufacture = $this->db->from('products_manufacture')->where('shop_id', $parent)->get()->result_array();
//        $stores_k = $this->db->from('stores')->where('shop_id', $parent)->get()->result_array();
    ...
    public function cms_vcrproduct()
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $sls_group = $this->db->from('products_group')->where('shop_id', $parent)->get()->result_array();

        $sls_manufacture = $this->db->from('products_manufacture')->where('shop_id', $parent)->get()->result_array();
        $store = $this->db->from('stores')->where('shop_id', $parent)->get()->result_array();
        $data['data']['_prd_group'] = $sls_group;
        $data['data']['_prd_manufacture'] = $sls_manufacture;
        $data['data']['_prd_store'] = $store;
        $data['data']['parent_id'] = $parent;
    ...
    public function cms_create_manufacture()
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $prd_manuf = $this->db->from('products_manufacture')->where('prd_manuf_name', $data['prd_manuf_name'])->where('shop_id', $data['shop_id'])->get()->row_array();
    ...
    public function cms_paging_manufacture($page = 1)
    {
        ...
        $data ['_list_prd_manuf'] = $this->db->from('products_manufacture')->where('shop_id', $parent)->limit($config['per_page'], ($page - 1) * $config['per_page'])->order_by('created', 'desc')->get()->result_array();
    ...
    public function cms_create_group()
    {
        ...
        if (isset($data['parentid']) && $data['parentid'] > 0) {
            $level = $this->db->select('level')->from('products_group')->where('shop_id', $data['shop_id'])->where('ID', $data['parentid'])->limit(1)->get()->row_array();
            $data['level'] = $level['level'] + 1;
            $prd_group = $this->db->from('products_group')->where('shop_id', $data['shop_id'])->where(['parentid' => $data['parentid'], 'prd_group_name' => $data['prd_group_name']])->get()->row_array();
        } else {
            $prd_group = $this->db->from('products_group')->where('shop_id', $data['shop_id'])->where(['parentid' => 0, 'prd_group_name' => $data['prd_group_name']])->get()->row_array();
        }
...
//        print_r($sls_group);die();
    ...
    public function cms_load_listmanufacture()
    {
        ...
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $this->cms_nestedset->set('products_group');
        $data = $this->db->from('products_manufacture')->where('shop_id', $parent)->order_by('created', 'desc')->get()->result_array();
        print_r($data);die();
        ob_start();
...
//        $data ['_list_prd_group'] = $this->cms_nestedset->where('shop_id', $parent)->data('products_group', NULL, ['per_page' => $config['per_page'], 'page' => $page]);
        $data ['_list_prd_group'] = $argc_s;
    ...
    public function cms_save_item_prdGroup($id)
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $prd_group = $this->db->from('products_group')->where('id', $id)->where('shop_id', $parent)->get()->row_array();
        if (empty($prd_group) && count($prd_group) == 0) {

            echo $this->messages = '0';
            return;
        }
        $prd_group_check = $this->db->from('products_group')->where([ 'prd_group_name' => $data['prd_group_name'],'shop_id' => $parent])->get()->row_array();
    ...
    public function cms_delete_Group($id)
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $prd_group = $this->db->where('id', $id)->where('shop_id', $parent)->from('products_group')->get()->row_array();
    ...
    {
        $data = $this->input->post('data');
        $parent_id = $data['prd_parent_id'];
        $store_id = $data['store_id'];
        $data = $this->cms_common_string->allow_post($data, ['prd_code', 'prd_parent_id', 'prd_name', 'prd_sls', 'prd_inventory', 'prd_allownegative', 'prd_origin_price', 'prd_sell_price', 'prd_group_id', 'prd_manufacture_id', 'prd_vat', 'prd_image_url', 'prd_descriptions', 'display_website', 'prd_new', 'prd_hot', 'prd_highlight']);
        $check_code = $this->db->select('ID')->from('products')->where('prd_code', $data['prd_code'])->where('shop_id', $parent_id)->get()->row_array();
        if (!empty($check_code) && count($check_code)) {
            ...
            if ($data['prd_code'] == '') {
                ...
                $max_product_code = $this->db->get_where('products',array('shop_id' => $parent_id))->row();
            ...
            unset($data['prd_parent_id']);
            unset($data['store_id']);
            $data['shop_id'] = $parent_id;
            $this->db->insert('products', $data);
            $product_id = $this->db->insert_id();
            $user_init = $data['user_init'];
            $inventory = ['store_id' => $store_id, 'product_id' => $product_id, 'quantity' => $quantity, 'user_init' => $user_init,'shop_id' => $parent_id];
            ...
            // $picture['url'] = 'dfsdf df';
            // $picture['product_id'] = 0;
            // $this->db->insert('picture', $picture);
    ...
    public function cms_paging_product($page = 1)
    {
            ...
            if ($option['option2'] == '-1') {
                if ($option['option3'] == '-1') {
                    $total_prd = $this->db
                        ...
                        ->where('products.shop_id', $option['parent_id'])
                        ->count_all_results();
        ...
        $data['data']['_sl_manufacture'] = $this->db->from('products_manufacture') ->where('shop_id', $option['parent_id'])->count_all_results();
