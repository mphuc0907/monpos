...
class Customer extends CI_Controller
{
    ...
    public function index($page = 1)
    {
        ...
        $total_customer = $this->db->from('customers')->where('shop_id',$parent)->count_all_results();
...
//        print_r($store);die();
    ...
    public function cms_detail_customer($id)
    {
        ...
        $store_id = $this->db->from('users')->where('id', $this->auth['id'])->limit(1)->get()->row_array();
        $parent = ($store_id['group_id'] == 1) ? 1 : (($store_id['parent_id'] == 1) ? $store_id['id'] : $store_id['parent_id']);
        $cus = $this->db->from('customers')->where('shop_id', $parent)->where('id', $id)->get()->row_array();
    ...
    public function cms_delCustomer()
    {
        ...
        if (!isset($customer) && count($customer) == 0) {
            ...
            $this->db->where('ID', $id)->where('shop_id', $parent)->delete('customers');
    ...
    public function cms_save_edit_customer($id)
    {
        ...
        $this->db->where('ID', $id)->where('shop_id', $parent)->update('customers', $data);
        echo $this->messages = '1';
    }
}