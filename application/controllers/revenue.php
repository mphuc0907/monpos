...
class Revenue extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $user = $this->db
            ...
            ->where(['deleted' => 0, 'order_status' => 1, 'orders.shop_id' => $parent])
            ->get()
            ->result_array();
        ...
        $sale = $this->db
            ...
            ->from('users')
            ->where('id', $parent)
            ->or_where('parent_id', $parent)
            ->get()
            ->result_array();
        $data['data']['sales'] = $sale;
        $customer = $this->db->from('customers')->where('shop_id', $parent)->get()->result_array();
    ...
    public function cms_paging_revenue($page = 1)
    {
        ...
        (int)$st = (int)(($page - 1) * $config['per_page']);
...
//                                    ->where('sale_id', $option['option4'])
            ...
            $config['per_page'] = 10;
            $this->pagination->initialize($config);
            $_pagination_link = $this->pagination->crceate_links();
            $data['total_orders'] = $total_orders;
            if ($page > 1 && ($total_orders['quantity'] - 1) / ($page - 1) == 10)
                $page = $page - 1;
