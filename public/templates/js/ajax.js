...
*/
    ...
    if (window.location.pathname.indexOf('config') !== -1) {
        ...
        var id = $('#create-nv .group-selbox').data('user_id');
        cms_upstore();
        cms_crfunc();
        cms_upfunc();
        cms_crgroup();
...
function cms_deltore($id) {
    "use strict";
    var conf = confirm('Bạn chắc chắn muốn xóa!');
    if (conf) {
        var $param = {
            'type': 'POST',
            'url': 'ajax/cms_deltore',
            'data': {'id': $id},
            'callback': function (data) {
                if (data != '0') {
                    cms_upstore();
                    $('.add_butn_them').html('  <i class="fa fa-users"></i> Danh sách kho <div class="action pull-right hide_kho_full">     <button class="btn btn-default btn-sm create-group btn-in-panel blue" data-toggle="modal"data-target="#create-store"><i class="fa fa-pencil blue"></i> Thêm kho </button> </div>')
                    // cms_radiogroup();
                } else {
                    alert('Không thể xóa kho khi đang có sản phẩm trong kho!');
                }
            }
        };
        cms_adapter_ajax($param);
...
function cms_uptore($id) {
    "use strict";
    var $stock_name_input = $('#stock_name_input').val();
    // var conf = confirm('Bạn chắc chắn muốn xóa!');
    // if (conf) {
    var $param = {
        'type': 'POST',
        'url': 'ajax/cms_up_store',
        'data': {'id': $id, 'stock_name_input': $stock_name_input},
        'callback': function (data) {
            if (data != '0') {
                cms_upstore();
                // cms_radiogroup();
            } else {
                alert('Không thể sửa tên nhóm được!');
            }
        }
    };
    cms_adapter_ajax($param);
...
function cms_create_manufacture($cont) {
    'user strict';
    var $shop_id = $.trim($('#prd_manuf_parent_id').val());
    ...
    if ($prd_manuf_name.length == 0) {
        ...
        var $param = {
            ...
            'data': {'data': {'prd_manuf_name': $prd_manuf_name, 'shop_id': $shop_id}},
            'callback': function (data) {
                if (data == '1') {
                    cms_paging_manufacture(1);
...
function cms_create_group($cont) {
    ...
    var $prd_group_parent_id = $.trim($('#prd_group_parent_id').val());
    var $parentid = $('#parentid').val();
    var $data = {'data': {'prd_group_name': $prd_group_name, 'parentid': $parentid, 'shop_id': $prd_group_parent_id}};
    console.log($data);
...
function cms_add_product(type) {
    'use strict';
    var $prd_parent_id = $('#prd_parent_id').val();
    var $store_id = $('#prd_store_id').val();
...
function cms_update_order($id) {
    var $param = {
        'type': 'POST',
        'url': 'orders/cms_update_order/',
        'data': {'id': $id},
        'callback': function (data) {
            $('.orders').html(data);
        }
...
function cms_paylack() {
    $id_bill_lack = $('#id_bill_lack').val();
    $lack_pay = $('#lack_pay').val();
    $status_pay = $('#id_status_pay').val();
    $data = {
        'data': {
            'id': $id_bill_lack,
            'lack_pay': $lack_pay,
    ...
    // console.log($('#id_status_pay').val());
    var $param = {
        'type': 'POST',
        'url': 'orders/cms_paylack/',
        'data':$data,
        'callback': function (data) {
            if (data.length != 0) {
                setTimeout(() => {
...
function cms_save_orders(type) {
    if ($('tbody#pro_search_append tr').length == 0) {
        ...
        var $param = {
            'type': 'POST',
            'url': 'orders/cms_save_orders/',
            'data': $data,
            'callback': function (data) {
                if (data == '0') {
                    $('.ajax-error-ct').html('Oops! This system is errors! please try again.').parent().fadeIn().delay(1000).fadeOut('slow');
                } else if (data == '10') {
                    $('.ajax-error-ct').html('Sản phẩm hiện không có hoặc đã hết trong kho hàng bạn chọn vui lòng chọn sản phẩm khác').parent().fadeIn().delay(1000).fadeOut('slow');
                } else {
                    if (type == 1) {
                        ...
                        $('.ajax-success-ct').html('Đã lưu thành công đơn hàng.').parent().fadeIn().delay(5000).fadeOut('slow');
                        setTimeout(function () {
                            $('.btn-back').delay('5000').trigger('click');
                        }, 3000);
...
function cms_paging_inventory($page) {
    ...
    var $param = {
        ...
        'data': $data,
        'callback': function (data) {
            /* codefly.vn add callback js _ <=> path*/
            $('.inventory-main-body-public_templates_js_ajax_js').html(data);
        }
    };
    cms_adapter_ajax($param);
}
...
function cms_loadListInvOption() {
    ...
    $('#option_stores').on('change', function () {
        cms_paging_inventory(1);
    });
...
//tao phieu thu chi
function cms_phieuchi() {
    'use strict';
    var $thuchi_code = $.trim($('#thuchi_code').val()); //thuchi_code, ma phieu
    //var $store_id = $.trim($('#store_id').val()); //store_id, kho chi
    //var $thuchi_date = $.trim($('#thuchi_date').val()); //thuchi_date, ngay chi
    //var $user_init = $.trim($('#user_init').val()); // user_init, nguoi chi
    var $notes = $.trim($('#notes').val()); // notes ghi chu
    var $hinhthuc = $('#hinhthuc').val(); // hinhthuc hinh thuc
    //var $loaiphieu = $.trim($('#loaiphieu').val()); //loaiphieu loai phieu
    var $tongtien = $.trim($('#tongtien').val()); //tongtien tong tien

    var $data = {
        'data': {
            'thuchi_code': $thuchi_code,
            //'store_id': $store_id,
            //'thuchi_date': $thuchi_date,
            //'user_init': $user_init,
            'notes': $notes,
            'hinhthuc': $hinhthuc,
            //'loaiphieu': $loaiphieu,
    ...
    if ($tongtien.length == 0) {
        $('.error-tongtien').text('Tổng tiền không được để trống.');
    } else {
        //$('.error-tongtien').text('');        

        var $param = {
            'type': 'POST',
            'url': 'phieuchi/cms_phieuchi',
            'data': $data,
            'callback': function (data) {
                if (data > 0) {
                    $('.btn-close').trigger('click');
                    $('.ajax-error-ct').hide();
                    $('.ajax-success-ct').html('Tạo thành công!').parent().fadeIn().delay(1000).fadeOut('slow');
                    cms_paging_listthuchi(1);
                    $("#search-box-mas").prop('readonly', true).attr('data-id', data).val($name);
                    $(".del-mas").html('<i class="fa fa-minus-circle" aria-hidden="true"></i>');
                } else {
                    if (data == 0) {
                        ...
                        $('.ajax-error-ct').html('Lỗi hệ thống.' + data).parent().fadeIn().delay(1000).fadeOut('slow');
                    }
...
function cms_delThuchi($id, $page) {
    'use strict';
    var conf = confirm('Bạn chắc chắn muốn xóa!');
    if (conf) {
        var $param = {
            'type': 'POST',
            'url': 'phieuchi/cms_delThuchi',
            'data': {'id': $id},
            'callback': function (data) {
                if (data == '0') {
                    $('.ajax-error-ct').html('Lỗi! không thể xóa').parent().fadeIn().delay(1000).fadeOut('slow');
                } else {
                    $('.ajax-success-ct').html('Bạn đã xóa khách hàng!').parent().fadeIn().delay(1000).fadeOut('slow');
                    cms_paging_listthuchi($page);
                }
            }
        };
        cms_adapter_ajax($param);
...
function cms_paging_listthuchi($page) {
    $keyword = $('#order-search').val();
    $option = $('#search-option-1').val();
    ...
    var $param = {
        'type': 'POST',
        'url': 'phieuchi/cms_paging_listthuchi/' + $page,
        'data': $data,
        'callback': function (data) {
            $('.phieuchi-main-body').html(data);
        }
    };
    cms_adapter_ajax($param);
}
...
//TAO PHIEU THU
//tao phieu thu chi
function cms_phieuchithu() {
    'use strict';
    var $thuchi_code = $.trim($('#thuchi_codethu').val()); //thuchi_code, ma phieu
    //var $store_id = $.trim($('#store_id').val()); //store_id, kho chi
    //var $thuchi_date = $.trim($('#thuchi_date').val()); //thuchi_date, ngay chi
    //var $user_init = $.trim($('#user_init').val()); // user_init, nguoi chi
    var $notes = $.trim($('#notesthu').val()); // notes ghi chu
    var $hinhthuc = $('#hinhthucthu').val(); // hinhthuc hinh thuc
    //var $loaiphieu = $.trim($('#loaiphieu').val()); //loaiphieu loai phieu
    var $tongtien = $.trim($('#tongtienthu').val()); //tongtien tong tien
...
function cms_delThuchiThu($id, $page) {
    'use strict';
    var conf = confirm('Bạn chắc chắn muốn xóa!');
    if (conf) {
        var $param = {
            'type': 'POST',
            'url': 'phieuthu/cms_delThuchiThu',
            'data': {'id': $id},
            'callback': function (data) {
                if (data == '0') {
                    $('.ajax-error-ct').html('Lỗi! không thể xóa').parent().fadeIn().delay(1000).fadeOut('slow');
...
function cms_paging_listthuchithu($page) {
    $keyword = $('#order-search').val();
    $option = $('#search-option-1').val();
    ...
    var $param = {
        'type': 'POST',
        'url': 'phieuthu/cms_paging_listthuchithu/' + $page,
        'data': $data,
        'callback': function (data) {
            $('.phieuchithu-main-body').html(data);
        }
    };
    cms_adapter_ajax($param);
}
